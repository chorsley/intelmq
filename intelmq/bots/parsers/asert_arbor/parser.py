...
class AsertArborParserBot(ParserBot):
    def processa(self):
        ...
        for incident in self.parse(report):
            self.logger.info(incident)
            event = Event(report)
            event["Impact"] = incident.get("IncidentID")
    ...
    def parsea(self, report):
        ...
        import pdb
        pdb.set_trace()
        yield {"IncidentID": "ajdk"}
        soup = BeautifulSoup(report, 'xml.parser')

        for incident in report.find_all("Incident"):
